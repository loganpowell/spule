                ...
                case 7:
                    ...
                    if (reso) {
                        ...
                        if (erro && result instanceof Error) {
                            ...
                            if (error.sub$)
                                return [2 /*return*/, stream_1.command$.next(error)];
                        ...
                        if (!(result instanceof Error)) {
                            ...
                            if (resolved.sub$)
                                stream_1.command$.next(resolved);
                            else if (!sub$)
                    ...
                    if (result !== Object(result)) {
                        ...
                        stream_1.command$.next((_d = {}, _d[keys_1.CMD_SUB$] = sub$, _d[keys_1.CMD_ARGS] = result, _d));
                        return [2 /*return*/, acc];
                    }
                    // if the result has made it this far, send it along
                    // console.log(`${sub$} made it through`)
                    stream_1.command$.next((_e = {}, _e[keys_1.CMD_SUB$] = sub$, _e[keys_1.CMD_ARGS] = result, _e));
                    return [2 /*return*/, __assign(__assign({}, acc), result)];
            }
        });
    }); }, Promise.resolve({}));
