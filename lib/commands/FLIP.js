...
var flip_toolkit_1 = require("flip-toolkit");
...
var FLIPLastInvertPlay = function (_a) {
    ...
    console.log({ Tx: Tx, Ty: Ty, Sx: Sx, Sy: Sy, x: x, y: y });
    window.scrollTo(x, y);
...
var log = console.log;
// @ts-ignore
var flipper = new flip_toolkit_1.Flipper({ element: document.body });
var FLIPF = function (_a) {
    var id = _a.id, target = _a.target;
    flipper.addFlipped({
        element: target,
        flipId: id,
        onStart: function () { return log("animation started"); },
        onSpringUpdate: function (springValue) { return log("current spring value: " + springValue); },
        onComplete: function () { return log("animation completed!"); }
    });
    flipper.recordBeforeUpdate();
};
var FLIPLIP = function (_a) {
...
exports.FLIP_FIRST = register_1.registerCMD((_a = {},
    ...
    _a[keys_1.CMD_WORK] = function (_a) {
        var id = _a.id, target = _a.target;
        return (
        // log({ id, target }), FLIPFirst({ id, target, state })
        log({ id: id, target: target }), FLIPF({ id: id, target: target }));
    },
    _a));
// init
exports.FLIP_LAST_INVERSE_PLAY = register_1.registerCMD((_b = {},
    ...
    _b[keys_1.CMD_WORK] = function (_a) {
        var id = _a.id, element = _a.element;
        return (
        // log({ id, element }), FLIPLastInvertPlay({ id, element, state })
        log({ id: id, element: element }), FLIPLIP({ id: id, element: element }));
    },
    _b));
