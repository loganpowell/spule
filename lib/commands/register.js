...
var feedCMD$fromSource$ = function (cmd) {
    ...
    var deliver = function (x) {
        var _a;
        return (_a = {}, _a[keys_1.CMD_SUB$] = sub$, _a[keys_1.CMD_ARGS] = args(x), _a);
    };
    var delivery = (_a = {}, _a[keys_1.CMD_SUB$] = sub$, _a[keys_1.CMD_ARGS] = args, _a);
    var feed = function ($) {
        return isFn ? transducers_1.map(function (x) { return $.next(deliver(x)); }) : transducers_1.map(function () { return $.next(delivery); });
    };
    return cmd[keys_1.CMD_SRC$].subscribe(feed(stream_1.command$));
};
...
function registerCMD(command) {
    ...
    // @ts-ignore
    stream_1.out$.subscribeTopic(sub$, { next: work, error: console.warn }, transducers_1.map(function (puck) { return puck[keys_1.CMD_ARGS]; }));
    var CMD = reso
        ? (_a = {},
